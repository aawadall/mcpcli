...
func loadMCPConfig(configPath string) (*core.MCPConfig, error) {
	...
	// Try to load as ProjectConfig first (the format generated by the CLI)
	var projectConfig core.ProjectConfig
	if err := json.Unmarshal(data, &projectConfig); err == nil {
		// Successfully loaded as ProjectConfig, convert to MCPConfig
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.TestAll || opts.TestResources {
				...
				if err != nil {
					...
					resourcesMap, ok := resp.Result.(map[string]interface{})
					if ok {
						resources, ok := resourcesMap["resources"].([]interface{})
						if ok {
							fmt.Printf("%s %sResources:%s\n", colorGreen, emojiCheck, colorReset)
							for _, r := range resources {
								if resMap, ok := r.(map[string]interface{}); ok {
									fmt.Printf("  - Name: %v, Type: %v, URI: %v\n", resMap["name"], resMap["type"], resMap["uri"])
								}
							}
						} else {
			...
			if opts.TestAll || opts.TestTools {
				...
				if err != nil {
					...
					toolsMap, ok := resp.Result.(map[string]interface{})
					if ok {
						tools, ok := toolsMap["tools"].([]interface{})
						if ok {
